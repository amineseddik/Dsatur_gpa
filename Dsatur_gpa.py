...
def main():
    ...
    st.markdown(
        ...
        .warning-text {
            color: #B22222; /* Rouge foncé pour les avertissements */
            font-size: 18px;
            font-weight: bold;
        }
        .subtitle {
            color: #1F618D; /* Bleu foncé */
            font-size: 20px;
            font-weight: bold;
            margin-top: 15px;
        }
    ...
    if st.button("Colorer le Graphe"):
        if any(graph.values()):
            result = dsatur(graph)
            st.markdown('<div class="subtitle">Coloration obtenue :</div>', unsafe_allow_html=True)
            result_text = "".join(
                [f"<p class='color-text'>Sommet {vertex:02d} → Couleur {color}</p>" for vertex, color in sorted(result.items())]
            )
            st.markdown(result_text, unsafe_allow_html=True)
            
            st.markdown('<div class="subtitle">Visualisation du graphe :</div>', unsafe_allow_html=True)
            plt = visualize_graph(graph, result)
            st.pyplot(plt)
            plt.close()
        else:
            st.markdown('<p class="warning-text">Veuillez définir les connexions du graphe avant de continuer.</p>', unsafe_allow_html=True)
